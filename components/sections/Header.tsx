    ...
    <>
      <header className="sticky top-0 z-50 bg-surface-elevated backdrop-blur-sm">
        <div className="flex items-center h-[60px] px-4 md:px-12 gap-3 md:gap-8 max-w-[1920px] mx-auto">
          {/* Logo */}
          <a href="#hero" aria-label="Region RP — на главную" className="flex-shrink-0">
            <Image src="/images/logo.svg" alt="Region RP" width={109} height={22} priority />
          </a>
            ...
            {NAV_ITEMS.map(({ label, href, id }) => (
              <a
                ...
                className={`text-nav font-semibold transition-colors duration-150 ${
                  activeId === id ? 'text-white' : 'text-text-muted hover:text-white'
                }`}
              >
                {label}
              </a>
          ...
          <div className="hidden min-[440px]:flex items-center gap-5 ml-auto">
            <Suspense fallback={<span className="w-32 h-4 rounded bg-white/10 animate-pulse" />}>
              <OnlineCounter count={onlineCount} />
            </Suspense>

            <Button variant="primary" size="sm" href="#" icon={<PersonIcon />}>ВОЙТИ</Button>
      ...
      <div
        ...
        style={{ background: 'radial-gradient(ellipse at 55% 15%, #6b3a4a 0%, #111117 65%)' }}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {/* Nav items — centered */}
        <nav className="flex flex-col items-center gap-10 mt-6" aria-label="Мобильная навигация">
          {NAV_ITEMS.map(({ label, href, id }) => (
            <a
              key={id}
              href={href}
