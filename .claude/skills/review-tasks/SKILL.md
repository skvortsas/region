---
name: review-tasks
description: Review a completed task from TASKS.md for production readiness. Takes a task ID (e.g. "3.A", "3B", "1.2") from the argument, reads the matching task in TASKS.md, audits the actual implementation against its acceptance criteria, and returns a 5-line review plus a confidence level (1-10). If confidence < 9, also returns a TBD plan listing what must change before the task is production-ready.
---

# /review-tasks

You have been invoked to audit a single task from `TASKS.md` against its actual implementation in the repo. The user gives you a task identifier in `$ARGUMENTS` (e.g. `3.A`, `3B`, `1.2`, `2.5`). Normalize variants like `3B` ↔ `3.A` ↔ `3.b` to the canonical form used in TASKS.md.
...
- Use the **opus** model for this review (you are already running on it via the skill invocation — no action needed, just don't delegate to a smaller model).
